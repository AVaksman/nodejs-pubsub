...
export class Snapshot {
  ...
  constructor(parent: Subscription|PubSub, name: string) {
    ...
    //    * });
    //    */
    //   this.create = (parent as Subscription).createSnapshot!.bind(parent, name);
    // }
    //   if (parent instanceof Snapshot) {
  ...
  create(gaxOpts?: CallOptions|CreateSnapshotCallback, callback?: CreateSnapshotCallback): void|Promise<CreateSnapshotResponse> {
    const con = this.parent instanceof Subscription;
    if (!(this.parent instanceof Subscription)) {
      throw new Error(`This is only available if you accessed this object through {@link Subscription#snapshot}`);
    }
    return (this.parent as Subscription).createSnapshot(this.name, gaxOpts! as CallOptions, callback!);
  }
  ...
  seek(gaxOpts: CallOptions, callback: google.pubsub.v1.Subscriber.SeekCallback): void;
  seek(gaxOpts?: CallOptions|google.pubsub.v1.Subscriber.SeekCallback, callback?: google.pubsub.v1.Subscriber.SeekCallback): void|Promise<google.pubsub.v1.SeekResponse> {
    if (!(this.parent instanceof Subscription)) {
      throw new Error(`This is only available if you accessed this object through {@link Subscription#snapshot}`);
    }
    return (this.parent as Subscription).seek(this.name, gaxOpts! as CallOptions, callback!);
  }
  ...
  delete(callback?: RequestCallback<google.protobuf.Empty>):
    ...
    (this.parent as PubSub).request<google.protobuf.Empty>(
        {
